...
import { getPrescription, consumePrescription, downloadPDF, Prescription } from '@/lib/api';
...
export default function PatientPrescriptionDetailPage() {
  ...
  const handleConsume = async () => {
    ...
    try {
      await consumePrescription(prescription.id);
      router.refresh();
    } catch (err) {
        ...
        <div className="bg-card shadow rounded-lg p-6">
          ...
          <div className="border-t pt-6 mt-6 flex gap-3">
            {prescription.status === 'pending' && (
              ...
              >
                {actionLoading ? '...' : 'Marcar como consumida'}
              </button>
            )}
            <button
